...
public:
	...
	void Move(float x, float y, float z) { m_fxPosition += x; m_fyPosition += y; m_fzPosition += z; }	// 게임 객체를 x-축, y-축, z-축으로 이동한다.
	void Rotate(float x, float y, float z) { m_fxRotation += x; m_fyRotation += y; m_fzRotation += z; }	// 게임 객체를 x-축, y-축, z-축으로 회전한다.

public:
	// 메쉬의 정점 하나를 게임 객체의 위치와 방향을 이용하여 월드 좌표 변환을 한다.
