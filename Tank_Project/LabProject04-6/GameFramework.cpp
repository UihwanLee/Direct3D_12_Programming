        ...
        case WM_KEYUP:
			switch (wParam) 
            {
				...
				case VK_F9:
					...
					break;
				case VK_SHIFT:
					((CAirplanePlayer*)m_pPlayer)->SetMissile();
					break;
				case VK_CONTROL:
					((CAirplanePlayer*)m_pPlayer)->TryFireMissile();
					break;
				default:
					break;
			} 
...
void CGameFramework::BuildObjects()
{
	...
	TankAI* pTankAI = new TankAI(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature());
	pTankAI->SetPosition(XMFLOAT3(0.0f, 0.0f, 0.0f));
	m_ppAITanks[0] = pTankAI;

	pTankAI = new TankAI(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature());
	pTankAI->SetPosition(XMFLOAT3(0.0f, 0.0f, -13.0f));
	pTankAI->Rotate(0.0f, 180.0f, 0.0f);
	m_ppAITanks[1] = pTankAI;

	m_pd3dCommandList->Close();
	ID3D12CommandList *ppd3dCommandLists[] = { m_pd3dCommandList };
	m_pd3dCommandQueue->ExecuteCommandLists(1, ppd3dCommandLists);

	m_pScene->SetObjects(m_pPlayer, m_nAITanks, m_ppAITanks);
...
void CGameFramework::ProcessInput()
{
	...
	((CAirplanePlayer*)m_pPlayer)->Update(m_GameTimer.GetTimeElapsed());
}
...
void CGameFramework::AnimateObjects()
{
	...
	for (int i = 0; i < m_nAITanks; i++)
	{
		((TankAI*)m_ppAITanks[i])->Animate(m_GameTimer.GetTimeElapsed());
	}
}
...
{    
	...
	for (int i = 0; i < m_nAITanks; i++)
	{
		((TankAI*)m_ppAITanks[i])->Render(m_pd3dCommandList, m_pCamera);
	}
