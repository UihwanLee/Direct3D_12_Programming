...
void CScene::SetObjects(CPlayer* pPlayer, int nAITanks, AI** ppAITanks)
{
	m_pPlayer = pPlayer;
	...
	playerInitPos = m_pPlayer->GetPosition();
}
...
void CScene::AnimateObjects(float fTimeElapsed)
{
	...
	CheckCollisionByBullet();

	CheckCollisionByGround();
}

void CScene::CheckCollisionByBullet()
{
	CMissileObject** ppMissiles = ((CAirplanePlayer*)m_pPlayer)->m_ppMissiles;

	for (int i = 0; i < m_nAITanks; i++)
	{

		// 적이 플레이어의 대포에 맞았는지 체크
		for (int j = 0; j < MISSILE; j++)
		{
			if (ppMissiles[j]->m_bFire)
			{
				if (CScene::CheckObjectByObjectCollisions(m_ppAITanks[i], ppMissiles[j], 2.0f))
				{
					// TO DO 
					ppMissiles[j]->Reset();
					if (((TankAI*)m_ppAITanks[i])->DecreaseHP(ppMissiles[j]->m_fMissileDamage) == false)
					{
...
bool CScene::CheckObjectByObjectCollisions(CGameObject* pGameObject, CGameObject* pTargetGameObject, float offset)
{
	// 오브젝트끼리의 위치관계로 충돌처리를 담당
	XMFLOAT3 pos = pGameObject->GetPosition();
	XMFLOAT3 targetPos = pTargetGameObject->GetPosition();
	if ((targetPos.x - offset < pos.x && targetPos.x + offset >= pos.x) && (targetPos.z - offset < pos.z && targetPos.z + offset >= pos.z)) return true;

	return false;
}

void CScene::CheckCollisionByGround()
{
	// 플레이어가 땅을 넘어가는지 체크
	XMFLOAT3 curPlayerPos = m_pPlayer->GetPosition();
	if (curPlayerPos.y < -3.0f)
	{
		m_pPlayer->SetPosition(playerInitPos);
	}
}
